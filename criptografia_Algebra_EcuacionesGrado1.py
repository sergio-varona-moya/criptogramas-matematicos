...
def genera_pdf(ruta_datos, ruta_elementos, valorLetraA=-13, conDenominadores=0, directorio_trabajo="./"):
    ...
    resultado = subprocess.run(["pdflatex", "--interaction=nonstopmode", "-output-directory=" + directorioFichas, rutaArchivoLaTeX],capture_output=True,text=True,cwd=os.path.dirname(os.path.abspath(__file__)))
    print("STDOUT pdflatex:", resultado.stdout)
    print("STDERR pdflatex:", resultado.stderr)
